...
def model_train(epoch,tokenizer,model,optimizer,dataloader,device):
...
def model_validate(epoch,tokenizer,model,dataloader,device,exp_len):
    ...
    with torch.no_grad():
        for i,data in enumerate(dataloader):
            ...
            generated_ids=model.generate(input_ids=ids,attention_mask=mask,max_length=exp_len,num_beams=2,
                length_penalty=1.0,repetition_penalty=2.0,early_stopping=True)
